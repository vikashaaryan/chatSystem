         ...
         class="py-3  bg-gray-800 rounded-2xl flex gap-4 cursor-pointer hover:bg-gray-700 transition relative {{ $conversation->id ==$selectedConversation?->id ? 'bg-gray-700':''}}">
            ...
            <a href="{{ route('chat',$conversation->id) }}" class="w-full">
                <div class="flex justify-between items-center">
                    <h6 class="truncate font-medium tracking-wider text-gray-200">{{ $conversation->GetReceiver()->name }}</h6>
                    <div class="flex items-center gap-2">

                        <small class="text-gray-400">{{ $conversation->messages?->last()?->created_at?->shortAbsoluteDiffForHumans() }}ago</small>
                ...
                <div class="flex gap-x-2 items-center border-b pb-2 border-gray-700">
                    <p class="truncate text-sm font-light grow text-gray-400">
                        Hey, what's up? Did you finish the task?
                    </p>
                    <span class="font-bold px-2 text-xs rounded-full bg-blue-600 text-white">2</span>
                </div>
            </a>
        </li>
